...
export const MetricsOverview: React.FC<MetricsOverviewProps> = ({
  ...
  const scriptJobsCount = metrics.scriptJobsCount ?? 3648;
  const scriptJobsSuccessRate = metrics.scriptJobsSuccessRate ?? 99.42;
  const scriptJobsDelta = metrics.scriptJobsDelta ?? 52;
  const scriptJobsRunning = metrics.scriptJobsRunningCount ?? 28;

  return (
    <section id="workbench-metrics-overview-stats" className="space-y-2">
      {/* Section Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <div 
          ...
          className="bg-white p-4 rounded-lg border border-[#E5E6EB] shadow-xs hover:border-[#FF6A00] hover:shadow-sm cursor-pointer transition-all group flex flex-col justify-between h-[148px]"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-3.5 bg-[#FF6A00] rounded-xs inline-block"></span>
          ...
          <div className="mt-2.5 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">个作业</span>
          ...
          <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <div className="flex items-center gap-1.5">
              ...
              <span>执行成功率:</span>
              <span className="font-semibold text-emerald-600 font-mono">{scriptJobsSuccessRate}%</span>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-3.5 bg-[#00B365] rounded-xs inline-block"></span>
              <span className="text-[13px] sm:text-sm text-slate-700 font-semibold tracking-tight">
                数据质量合格率
              </span>
            </div>
        ...
        {/* 3. API累计共享次数 */}
        <div className="bg-white p-4 rounded-lg border border-[#E5E6EB] shadow-xs hover:border-[#FF6A00] hover:shadow-sm transition-all group flex flex-col justify-between h-[148px]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-3.5 bg-[#1677FF] rounded-xs inline-block"></span>
              <span className="text-[13px] sm:text-sm text-slate-700 font-semibold tracking-tight">
                API累计共享次数
              </span>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-3.5 bg-[#722ED1] rounded-xs inline-block"></span>
              <span className="text-[13px] sm:text-sm text-slate-700 font-semibold tracking-tight">
                调度任务实例
              </span>
            </div>
